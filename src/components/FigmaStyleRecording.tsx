...
const FigmaStyleRecording: React.FC = () => {
  ...
  const addScreenRecording = async () => {
    ...
    const sourceId = await invoke('add_capture_source', {
      sourceType: 'monitor',
      deviceId: 'screen',
      name: `Screen ${captureSources.filter(s => s.source_type === 'monitor').length + 1}`,
  ...
  const handlePositionChange = async (sourceId: string, x: number, y: number) => {
    ...
    try {
      await invoke('update_capture_source_position', {
        ...
        x,
        y,
        width: captureSources.find(s => s.id === sourceId)?.width || 0,
        height: captureSources.find(s => s.id === sourceId)?.height || 0
      });
    } catch (error) {
  ...
  // Handle size change
  const handleSizeChange = async (sourceId: string, width: number, height: number) => {
    setCaptureSources(prev => prev.map(s => 
      s.id === sourceId ? { ...s, width, height } : s
    ));
    ...
    try {
      await invoke('update_capture_source_position', {
        sourceId,
        x: captureSources.find(s => s.id === sourceId)?.x || 0,
        y: captureSources.find(s => s.id === sourceId)?.y || 0,
        width,
        height
      });
    } catch (error) {
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-2xl font-bold text-gray-900">🎬 Figma-Style Recording Studio</h1>
              <p className="text-sm text-gray-600 mt-1">Drag windows like Figma elements - smooth and precise</p>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 rounded-lg border border-blue-200 p-4">
              ...
              <ul className="text-xs text-blue-800 space-y-1">
                <li>• Click and drag windows to move them</li>
                <li>• Drag blue handles to resize</li>
                <li>• Windows snap to container bounds</li>
                <li>• Smooth Figma-like interactions</li>
              </ul>
            </div>
          </div>
              ...
              >
                ...
                {captureSources.map((source) => (
                  <DraggableVideoWindow
                    ...
                    id={source.id}
                    name={source.name}
                    sourceType={source.source_type as 'monitor' | 'webcam'}
                    x={source.x}
                    y={source.y}
                    width={source.width}
                    height={source.height}
                    videoStream={source.source_type === 'monitor' ? screenStream : webcamStream}
...
export default FigmaStyleRecording;
